...
\section{Supplementary Discussion 1. Knee points across identification algorithms}

Figure \ref{fig:severson_knee_eol_all_algorithms} presents linear regressions of various knee point estimation algorithms for the capacity curves of the Severson et al. \cite{severson_data-driven_2019} dataset (except the Zhang et al.\cite{zhang_identifying_2020} method). We mention that the true knee point has no ``ground truth''.
For the Bacon-Watts and Kneedle identified knees, the linear regression holds with a goodness-of-fit $R^2\approx 99\%$ (and reduced variance between fit and residuals). With this metric, close by is the bisector\cite{greenbank_automated_2021} identified knees with $R^2\approx 96\%$. The tangent-ratio\cite{diao_algorithm_2019} identified knees show a higher fit-to-residuals variability.  
...
\section{Supplementary Discussion 3. Knees across identification algorithms}
