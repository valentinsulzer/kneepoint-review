...
%DIF DEL main.tex      Thu Mar 24 06:52:00 2022
%DIF ADD main_v2.tex   Mon Apr  4 03:11:30 2022
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% BIBLIOGRAPHY EXPERIMENTS %% See entry [141] for an et al example
% \usepackage[maxbibnames=99]{biblatex} %% This fumbles :(
...
(c) Capacity retention vs.~cycle number for lithium iron phosphate/graphite cells cycled at varying depth of discharge (DOD). \DIFdelbeginFL \DIFdelFL{Adopted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Adapted }\DIFaddendFL from Figure 3 of Wang et al.\cite{wang_cycle-life_2011}
...
\DIFdelbeginFL \DIFdelFL{Adopted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Adapted }\DIFaddendFL from Figure 4 of Wang et al.\cite{wang_cycle-life_2011}
}
\label{fig:x_axis}
\end{figure}
...
\DIFaddbegin \DIFadd{We detail these methods in Supplementary Discussion 1.
}\DIFaddend In Figure \ref{fig:knee_identification_methods}, we implemented and applied these methods to the capacity curve from a single cell in the Severson et al.\cite{severson_data-driven_2019} dataset.
\DIFdelbegin \DIFdel{The well-accepted ``Kneedle'' method (Figure \ref{fig:knee_identification_methods}a), proposed by Satop}%DIFDELCMD < {%%%
\DIFdel{\"a}%DIFDELCMD < }{%%%
\DIFdel{\"a}%DIFDELCMD < } %%%
\DIFdel{et al.\mbox{%DIFAUXCMD
\cite{satopaa_finding_2011}{}}\hspace{0pt}%DIFAUXCMD
, calculates the knee as the maximum distance of the aging trajectory from a line drawn from beginning to end of life.
The most common approaches use the intersection of two lines fit to the beginning and end of the aging trajectory.
The ``Bacon-Watts'' method (Figure \ref{fig:knee_identification_methods}b), proposed by Fermín-Cueto et al. \mbox{%DIFAUXCMD
\cite{fermin-cueto_identification_2020} }\hspace{0pt}%DIFAUXCMD
for use in battery aging trajectories, estimates the transition between two intersecting lines fitted to an aging trajectory; this method also provides an estimate of the ``knee-onset'', or the point where the aging trajectory is no longer linear.
The ``tangent-ratio'' method (Figure \ref{fig:knee_identification_methods}c), proposed by Diao et al.\mbox{%DIFAUXCMD
\cite{diao_algorithm_2019}{}}\hspace{0pt}%DIFAUXCMD
, defines the knee based on a tangent ratio at the inflection and maximum slope points of the aging trajectory.
Similarly, the bisector method (Figure \ref{fig:knee_identification_methods}d), proposed by Greenbank and Howey\mbox{%DIFAUXCMD
\cite{greenbank_automated_2021}{}}\hspace{0pt}%DIFAUXCMD
, combines linear extrapolations of early and late life with an angle bisector to identify the knee.
A final method was designed specifically for the battery use case: Zhang et al.\mbox{%DIFAUXCMD
\cite{zhang_accelerated_2019} }\hspace{0pt}%DIFAUXCMD
proposed the quantile regression method (Figure \ref{fig:knee_identification_methods}e), which approximates early life with a linear regression and then defines the knee as when the aging trajectory falls below a defined band below that regression line.
Unlike other methods that use only the aging trajectory, this method requires the use of voltage data.
For each of these methods, smoothing the aging trajectories prior to knee identification may result in more accurate or consistent knee point detection\mbox{%DIFAUXCMD
\cite{strange_elbows_2021}{}}\hspace{0pt}%DIFAUXCMD
, but again,
the results may be sensitive to the smoothing choices made.
We mention that Satop}%DIFDELCMD < {%%%
\DIFdel{\"a}%DIFDELCMD < }{%%%
\DIFdel{\"a}%DIFDELCMD < } %%%
\DIFdel{et al.\mbox{%DIFAUXCMD
\cite{satopaa_finding_2011} }\hspace{0pt}%DIFAUXCMD
review additional knee point identification algorithms that were not considered here.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!tb]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=1]{final_figures/knee_identification_methods.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Results of various knee identification methods proposed in the literature illustrated on the batch 2, channel 12 cell from the Severson et al.\mbox{%DIFAUXCMD
\cite{severson_data-driven_2019} }\hspace{0pt}%DIFAUXCMD
dataset (arbitrarily selected). The capacity is normalized by the nominal capacity of the cell.
(a) Kneedle method, proposed by Satop}%DIFDELCMD < {%%%
\DIFdelFL{\"a}%DIFDELCMD < }{%%%
\DIFdelFL{\"a}%DIFDELCMD < } %%%
\DIFdelFL{et al.\mbox{%DIFAUXCMD
\cite{satopaa_finding_2011}
}\hspace{0pt}%DIFAUXCMD
(b) Bacon-Watts method, proposed by Fermín-Cueto et al. \mbox{%DIFAUXCMD
\cite{fermin-cueto_identification_2020} }\hspace{0pt}%DIFAUXCMD
for use in battery aging trajectories.
(c) Tangent-ratio method, proposed by Diao et al.\mbox{%DIFAUXCMD
\cite{diao_algorithm_2019}
}\hspace{0pt}%DIFAUXCMD
(d) Bisector method, proposed by Greenbank and Howey.\mbox{%DIFAUXCMD
\cite{greenbank_automated_2021}
}\hspace{0pt}%DIFAUXCMD
(e) Quantile regression method, proposed by Zhang et al.\mbox{%DIFAUXCMD
\cite{zhang_accelerated_2019}}\hspace{0pt}%DIFAUXCMD
.
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:knee_identification_methods}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We compare the knee points as estimated by these five offline methods in Figure \ref{fig:knee_identification_methods}\DIFdelbegin \DIFdel{f}\DIFdelend .
All methods estimate the knee point at cycle numbers within a 26 cycle range (365--391 cycles). 
\DIFdelbegin \DIFdel{In general}\DIFdelend \DIFaddbegin \DIFadd{Looking across the Severson et al.\mbox{%DIFAUXCMD
\cite{severson_data-driven_2019} }\hspace{0pt}%DIFAUXCMD
dataset}\DIFaddend , the knee points estimated by these methods were highly correlated across most cells \DIFdelbegin \DIFdel{in the Severson et al.\mbox{%DIFAUXCMD
\cite{severson_data-driven_2019} }\hspace{0pt}%DIFAUXCMD
dataset }\DIFdelend (Figure S1; quantile regression method not included).
\DIFdelbegin \DIFdel{The Kneedle and Bacon-Watts knee points were highly correlated with each other ($R^2\approx 1.00$); the bisector method also correlated well to these methods ($R^2\approx 0.96$), but the tangent-ratio method correlated fairly poorly to the others ($R^2\approx 0.86$).
}\DIFdelend These results suggest that \DIFdelbegin \DIFdel{the Kneedle, Bacon-Watts, and bisector }\DIFdelend \DIFaddbegin \DIFadd{all of these }\DIFaddend methods are generally comparable for offline knee point estimation.
\DIFdelbegin \DIFdel{Of these five methods, the Kneedle, Bacon-Watts, and bisector methods are arguably also the easiest to implement, since all avoid the use of derivatives and voltage data.
}\DIFdelend 

\DIFaddbegin \begin{figure}[h!tb]
\centering
\includegraphics[scale=1]{final_figures/knee_identification_methods.pdf}
\caption{\DIFaddFL{Results of various knee identification methods proposed in the literature illustrated on the batch 2, channel 12 cell from the Severson et al.\mbox{%DIFAUXCMD
\cite{severson_data-driven_2019} }\hspace{0pt}%DIFAUXCMD
dataset (arbitrarily selected). The capacity is normalized by the nominal capacity of the cell.
(a) Kneedle method, proposed by Satop}{\DIFaddFL{\"a}}{\DIFaddFL{\"a}} \DIFaddFL{et al.\mbox{%DIFAUXCMD
\cite{satopaa_finding_2011}
}\hspace{0pt}%DIFAUXCMD
(b) Bacon-Watts method, proposed by Fermín-Cueto et al. \mbox{%DIFAUXCMD
\cite{fermin-cueto_identification_2020} }\hspace{0pt}%DIFAUXCMD
...
\DIFaddend Finding the knee ``online'' is difficult because the end-of-life capacity profile is not known and because the discharging conditions are often inconsistent.
...
(b) Electrode saturation, in which the number of active sites in the electrode \DIFdelbeginFL \DIFdelFL{constrains }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{has decreased and can no longer accommodate }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{incoming }\DIFaddendFL lithium inventory.
...
\textit{Snowball} trajectories (Figure \ref{fig:snowball_vs_hidden_vs_threshold}a and \ref{fig:snowball_vs_hidden_vs_threshold}d) occur when the underlying state variable has a direct relationship with the observable state variable, but the underlying state variable is \DIFdelbegin \DIFdel{exponentially increasing .
}\DIFdelend \DIFaddbegin \DIFadd{increasing superlinearly (e.g., the functional form is exponential, quadratic, cubic, etc.).
}\DIFaddend Positive feedback between two degradation mechanisms is a special case of a snowball trajectory, a point we return to in our discussion of interactions and heterogeneity.
...
\begin{figure}[htp]
    ...
    \caption{Schematic of the three types of ``internal state trajectories'' leading to a knee. In each case, the retention curve looks the same (a--c), but the underlying internal trajectories lead to knees via different mechanisms. (d) ``Snowball'' trajectory, in which the state variable is \DIFdelbeginFL \DIFdelFL{exponentially }\DIFdelendFL increasing \DIFaddbeginFL \DIFaddFL{via a superlinear functional form (e}\DIFaddendFL .\DIFaddbeginFL \DIFaddFL{g., exponential, quadratic, cubic, etc.). }\DIFaddendFL (e) ``Hidden'' trajectory, in which a slowly-increasing state variable (state 1) is dominated by a rapidly-increasing state variable (state 2). (f) ``Threshold'' trajectory, in which a dramatic change in observable state is triggered by a state variable reaching a threshold.
...
For instance, snowball trajectories require extrapolation of \DIFaddbegin \DIFadd{a superlinear function (e.g., }\DIFaddend an exponential function\DIFaddbegin \DIFadd{)}\DIFaddend , which is often an error-prone exercise and is exacerbated by noisy measurements.
...
Figure \ref{fig:summary} displays the connection between the six knee pathways and the three internal state trajectories.
\DIFdelbegin \DIFdel{Some knee }\DIFdelend \DIFaddbegin \DIFadd{We elaborate on each internal state trajectories for each knee pathway in the subsequent discussion; some knee }\DIFaddend pathways correspond to multiple internal state trajectories if multiple degradation mechanisms can occur for a pathway (e.g., lithium plating).
In theory, similar modeling and prediction approaches can apply to pathways with the same internal state trajectories.
In this section, we discuss the challenges and opportunities for modeling and prediction for each pathway and internal state trajectory.

\DIFdelbegin %DIFDELCMD < \begin{figure}[p]
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure}[htp]
    \DIFaddendFL \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.85]{final_figures/pathway_trajectory_summary.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.9]{final_figures/pathways_and_trajectories.pdf}
    \DIFaddendFL \caption{
    ...
    \DIFaddbeginFL \DIFaddFL{We elaborate on each internal state trajectories for each knee pathway in the subsequent discussion.
    }\DIFaddendFL }
    \label{fig:summary}
\end{figure}
...
Furthermore, lithium plating can occur \DIFdelbegin \DIFdel{either reversibly or irreversibly, depending on }\DIFdelend \DIFaddbegin \DIFadd{with various extents of reversibility within a cycle, defined as }\DIFaddend the ratio of lithium plated during charge that is recovered in the subsequent discharge \DIFaddbegin \DIFadd{(this ratio is always less than 100\%, meaning some irreverisble lithium plating always occurs)}\DIFaddend .\cite{baure_synthetic_2019, dubarry_big_2020}
In contrast to irreversible plating, reversible plating does not contribute to long-term degradation. Thus, we use ``plating'' to refer to irreversible plating throughout the remainder of this discussion.
...
For instance, Deichmann et al.\cite{deichmann_investigating_2020} created cells with $n$:$p$ ratios of 0.75 and 0.5 to intentionally deposit lithium metal on graphite electrodes. The authors identified a relationship between decreased $n$:$p$ and capacity fade, which they attributed to high loss of lithium inventory using differential capacity analysis and scanning electron microscopy. In a creative study, Martin et al.\cite{martin_cycling_2020} used deposited lithium metal as a mechanism to store extra capacity, enabling the cell to occasionally discharge extra energy (i.e., when extra range is needed) without requiring a substantially larger negative electrode. This cell design used an $n$:$p$ ratio of 0.6, where $n$:$p$ is calculated using the lithium capacity of the conventional graphite.
A high upper cutoff voltage during charging was used to intentionally plate lithium onto graphite; unsurprisingly, irreversible lithium plating was found to be the primary failure mechanism, with over 50\% capacity loss in two of the three electrolytes tested (although the cells did not exhibit knees).
Rate-independent lithium plating in fresh cells is trivial to model and predict if the cell design is known; if rate-independent plating is expected in fresh cells, a snowballing lithium plating knee \DIFdelbegin \DIFdel{is likely to }\DIFdelend \DIFaddbegin \DIFadd{(or, at a minimum, high early-in-life capacity loss) may }\DIFaddend occur early in life.
...
\DIFdelbegin \DIFdel{Various }\DIFdelend \DIFaddbegin \DIFadd{In principle, various }\DIFaddend degradation mechanisms can lead to $\mathrm{LAM_{deNE}}$, which occurs when active sites lose either ionic or electronic connectivity with the electrode.
\DIFdelbegin \DIFdel{Often}\DIFdelend \DIFaddbegin \DIFadd{Additionally}\DIFaddend , several of these mechanisms can occur in parallel, leading to a snowball effect where loss of active material due to one mechanism may result in further stress on the remaining active sites, accelerating degradation via rate-independent lithium plating.
...
\DIFaddbegin \DIFadd{However, we note that active material loss from well-made, graphitic negative electrodes cycling under ``reasonable'' conditions is often low in practice\mbox{%DIFAUXCMD
\cite{fathi_ultra_2014, harlow_wide_2019, cheng_effect_2020, gauthier_how_2022}}\hspace{0pt}%DIFAUXCMD
, so this effect may be most pronounced for cells with poorly made negative electrodes and/or cells with appreciable silicon content in the negative electrode. Furthermore, since negative electrode active material loss must outpace lithium inventory loss for this mechanism to occur, and since the rate of lithium inventory loss is more sensitive to high temperatures ($\sim$40}\degree \DIFadd{C) than active material loss\mbox{%DIFAUXCMD
\cite{harlow_wide_2019, gauthier_how_2022}{}}\hspace{0pt}%DIFAUXCMD
, this mechanism is more likely to occur at lower temperatures than higher temperatures.
}\DIFaddend 
...
however, \DIFaddbegin \DIFadd{since this technique typically requires low-rate cycling data, }\DIFaddend a major complication \DIFdelbegin \DIFdel{is estimating if the increased local current density from rate-independent estimates of }\DIFdelend \DIFaddbegin \DIFadd{to estimating if rate-dependent }\DIFaddend $\mathrm{LAM_{deNE}}$ is high enough to drive lithium plating \DIFaddbegin \DIFadd{is combining the rate-independent estimate from differential capacity analysis with the rate-dependent kinetic effects}\DIFaddend .
...
The plated lithium, which has a much lower density than intercalated lithium, further decreases the porous volume fraction, creating a positive feedback loop for additional lithium plating\cite{yang_modeling_2017} (Figure \DIFdelbegin \DIFdel{\ref{fig:plating_pathways}c}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:pore_clogging}a}\DIFaddend ).
...
A few works have modeled this phenomenon\cite{yang_modeling_2017, muller_model-based_2019, keil_electrochemical_2020, atalay_theory_2020}{}, namely Yang et al.\cite{yang_modeling_2017} (Figure \ref{fig:pore_clogging}\DIFaddbegin \DIFadd{b--c}\DIFaddend ).
While this mechanism has not been experimentally validated, decreasing negative electrode porosity with cycling has been \DIFaddbegin \DIFadd{conclusively }\DIFaddend observed via X-ray computed tomography\cite{frisco_understanding_2016, rahe_nanoscale_2019} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{(Figure \ref{fig:pore_clogging}d--e) and inconclusively observed via }\DIFaddend electrochemical impedance spectroscopy\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{klett_uneven_2015, petzl_lithium_2015, sarasketa-zabala_understanding_2015, sarasketa-zabala_cycle_2015}{}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{klett_uneven_2015, petzl_lithium_2015, sarasketa-zabala_understanding_2015, sarasketa-zabala_cycle_2015} }\hspace{0pt}%DIFAUXCMD
(attribution of spectral shifts to specific underlying phenomena, particularly via full cell electrochemical impedance spectroscopy, is challenging)}\DIFaddend .
...
\begin{figure}[hp]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=1]{final_figures/yang_remade.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.88]{final_figures/porosity_to_plating.pdf}
    \DIFaddendFL \caption{Rate-dependent lithium plating due to pore clogging\DIFdelbeginFL \DIFdelFL{, as modeled }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{.
    (a) Schematic illustration of pore clogging driven initially }\DIFaddendFL by \DIFaddbeginFL \DIFaddFL{SEI growth and then by plating. Adapted from Figure 8 of }\DIFaddendFL Yang et al.\cite{yang_modeling_2017}
    (\DIFdelbeginFL \DIFdelFL{a}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{b}\DIFaddendFL ) Lithium inventory loss contributed by SEI growth and lithium plating, respectively\DIFaddbeginFL \DIFaddFL{, as modeled by Yang et al}\DIFaddendFL .\DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{yang_modeling_2017}
    }\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{b}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{c}\DIFaddendFL ) Lithium inventory loss per cycle contributed by SEI growth and lithium plating, respectively\DIFaddbeginFL \DIFaddFL{, as modeled by Yang et al}\DIFaddendFL .\DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{yang_modeling_2017} }\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL The ``snowballing'' growth of lithium plating occurs due to the accelerating decrease in porosity, which creates high transport overpotentials in the electrolyte and drives additional lithium plating.
    (\DIFdelbeginFL \DIFdelFL{c}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{d}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{Schematic illustration }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Pore size distributions }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{pore clogging driven initially by SEI growth }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pristine }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{then by plating}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cycled subsections of graphitic negative electrode, as quantified via X-ray tomography phase contrast images}\DIFaddendFL . Adapted from Figure \DIFdelbeginFL \DIFdelFL{8 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3c }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{Yang }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Frisco }\DIFaddendFL et al.\DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\cite{yang_modeling_2017}}\hspace{0pt}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{frisco_understanding_2016}
    }\hspace{0pt}%DIFAUXCMD
(e) Comparison of the pristine and cycled samples via orthogonal virtual slices of the reconstructed X-ray phase contrast images. Reproduced from Figure 2c--d of Frisco et al.\mbox{%DIFAUXCMD
\cite{frisco_understanding_2016} }\hspace{0pt}%DIFAUXCMD
...
An open question is if (or under what conditions) SEI growth will cause lithium plating via decreased porosity or increased charge-transfer resistance first.
...
When the aging parameters (SEI reaction rate and particle cracking rate) are chosen so that loss of active material in the positive electrode occurs at a faster rate than loss of lithium inventory \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(e.g., compare Figure \ref{fig:electrode_sat_simulation}c to \ref{fig:electrode_sat_simulation}d), }\DIFaddend the stoichiometric window of the positive electrode widens (Figure \ref{fig:electrode_sat_simulation}f), which increases the cell voltage for a given amount of transferred lithium. This effect decreases the capacity between the voltage limits. The knee occurs when the positive electrode becomes fully saturated before the entire lithium inventory is transferred (around cycle 600 in Figure \ref{fig:electrode_sat_simulation}), despite the underlying rate of degradation remaining linear (Figure \ref{fig:electrode_sat_simulation}b-d).
\DIFaddbegin \DIFadd{This scenario may occur in cells with unstable positive electrodes, such as nickel-rich materials\mbox{%DIFAUXCMD
\cite{myung_nickel-rich_2017, manthiram_perspective_2017, laszczynski_understanding_2019} }\hspace{0pt}%DIFAUXCMD
or polycrystalline ternary oxide materials\mbox{%DIFAUXCMD
\cite{harlow_wide_2019}}\hspace{0pt}%DIFAUXCMD
, cycling under conditions such that the rate of positive electrode material loss exceeds the rate of lithium inventory loss.
}\DIFaddend 
...
Cell internal resistance often increases during aging, in part due to the growth of side reaction products on the surface of the electrode particles\DIFdelbegin \DIFdel{, particularly on the positive electrode \mbox{%DIFAUXCMD
\cite{ma_editors_2019}}\hspace{0pt}%DIFAUXCMD
.}\DIFdelend \DIFaddbegin \DIFadd{. This effect is most pronounced for oxide-based positive electrode materials like NMC, nickel cobalt aluminum oxide (NCA), lithium cobalt oxide (LCO), and lithium manganese oxide (LMO), as they operate well above the stability window of the electrolyte.\mbox{%DIFAUXCMD
\cite{ma_editors_2019,laszczynski_understanding_2019} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend Under constant current conditions, the additional overpotential from increased internal resistance will cause the cell to reach the cutoff voltage more quickly, decreasing the capacity, energy, and power per cycle. The magnitude of this overpotential growth rate is a product of both the resistance growth rate (i.e., electrolyte reaction rate) and the applied current.
...
This knee pathway is also sensitive to electrode chemistry, as each chemistry exhibits a unique voltage-capacity curve. For instance, lithium iron phosphate (LFP)/graphite cells experiencing high resistance growth would exhibit much sharper knees than NMC/graphite cells due to their flatter voltage-capacity curves. While LFP cells generally do not exhibit high resistance growth due to their lower \DIFdelbegin \DIFdel{operating voltages}\DIFdelend \DIFaddbegin \DIFadd{positive electrode operating voltages (well above the electrolyte stability window)}\DIFaddend \cite{keil_calendar_2016, safari_aging_2011}, even moderate resistance growth coupled with high discharge rates and high lower cutoff voltages could lead to dramatic cell failure (although the magnitude of this effect depends on the cutoff voltages).
...
\begin{figure}[ht!]
\centering
\includegraphics[scale = 1.0]{final_figures/CoveringLayers.pdf}
...
This pathway can be considered a threshold trajectory, where the jelly roll is eventually unable to accommodate the increase in internal volume and pressure. Modeling and predicting this mechanism, however, requires measuring and tracking the internal pressure over life, as well as knowing the internal pressure at which the jelly roll will deform. A center pin appears to help avoid this failure mode\DIFdelbegin \DIFdel{.\mbox{%DIFAUXCMD
\cite{waldmann_mechanical_2014, carter_mechanical_2019}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{waldmann_mechanical_2014, carter_mechanical_2019}}\hspace{0pt}%DIFAUXCMD
, although we note that center pins may serve other purposes such as facilitating gas flow along the cylindrical cell axis during a safety event or creating an additional electrolyte reservoir\mbox{%DIFAUXCMD
\cite{pfrang_long-term_2018}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Another interaction effect is resistance growth: since each of these pathways has some mechanism for increased resistance (most commonly via active material loss increasing the local current density and thus increasing the effective cell resistance), increased resistance growth caused by one pathway may exacerbate internal state variable growth in another pathway.
}\DIFaddend Given the high sensitivity of snowball pathways to small changes in state, interacting knee pathways can create positive feedback mechanisms with high sensitivity to internal state.
...
Lastly, the formation protocol can influence the location of the knee. For instance, Weng et al.\cite{weng_predicting_2021} found that NMC/graphite cells formed with a fast formation protocol that emphasizes time at high SOCs exhibited later knees than cells formed with a slower baseline formation protocol, which was attributed to the creation of well-passivating SEI at high potentials to decrease the amount of side reaction product formed during use.\cite{attia_benefits_2021}
...
Keil et al.\cite{keil_linear_2019} found that increasing discharging current from 1C to 2C led to the elimination of the knee in nickel manganese cobalt oxide (NMC)/graphite cylindrical cells \DIFaddbegin \DIFadd{at 25}\degree \DIFadd{C }\DIFaddend (Figure \ref{fig:discharge-rest_cycle}b).
Similarly, Atalay et al.\cite{atalay_theory_2020} found that increasing the discharge rate from 1C to 4C \DIFaddbegin \DIFadd{at 25}\degree \DIFadd{C }\DIFaddend decelerated the knee point for nickel cobalt aluminum oxide (NCA)/graphite cylindrical cells.
Lastly, Keil et al.\cite{keil_charging_2016} illustrated how discharging current had no effect on cylindrical cells with blended transition metal oxide positive electrodes and graphite negative electrodes \DIFaddbegin \DIFadd{at 25}\degree \DIFadd{C}\DIFaddend , but a lower discharging current (3A, 2.7C) led to faster degradation than a higher discharging current (5A, 4.5C) for an LFP/graphite cylindrical cell when charged at 4.5C \DIFaddbegin \DIFadd{and cycled at 25}\degree \DIFadd{C}\DIFaddend .
...
In other words, cells with less time spent cycling simply have less calendar aging.
\DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{For instance, in }\DIFaddend Figure S2b, we revisualized the Keil et al.\cite{keil_linear_2019} dataset \DIFaddbegin \DIFadd{(}\DIFaddend shown in Figure \ref{fig:discharge-rest_cycle}b\DIFaddbegin \DIFadd{) }\DIFaddend using estimated time on the $x$ axis\DIFdelbegin \DIFdel{; we }\DIFdelend \DIFaddbegin \DIFadd{.
We }\DIFaddend found that the knee locations appeared closer together, suggesting calendar aging is at least partially responsible for the discharge rate sensitivity.
\DIFaddbegin \DIFadd{In general, if the onset of the knee is delayed at higher discharging rates, decreased calendar aging may explain some or all of the difference.
}\DIFaddend This hypothesis further highlights the sensitivity of the apparent severity of the knee to the choice of $x$ axis (as illustrated in Figure \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:x_axis}}\DIFaddend ).

\subsubsection{Voltage limits} 
A wider voltage window generally accelerates the onset of the knee point \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ecker_calendar_2014, pfrang_long-term_2018, klett_non-uniform_2014, ma_novel_2019, petzl_lithium_2015, schuster_nonlinear_2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ecker_calendar_2014, klett_non-uniform_2014, petzl_lithium_2015, schuster_nonlinear_2015, pfrang_long-term_2018, ma_novel_2019}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . In one of the broadest studies, Ecker et al. \cite{ecker_calendar_2014} considered six DODs (100, 80, 50, 20, 10, and 0.5\%) with up to six voltage windows per DOD. The authors found that the EFC systematically decreased with increased DOD (Figure \ref{fig:ecker_capacity_and_resistance}). By 1000 EFC, all cells with DODs greater than 25-75\% had a capacity below 80\% and exhibited a knee. When varying the voltage window with fixed DOD, the authors observed the highest degradation in cells cycled at low and high SOCs; the lowest degradation was observed for a midpoint SOC of 50\%.
\DIFaddbegin \DIFadd{Keil and Jossen\mbox{%DIFAUXCMD
\cite{keil_charging_2016} }\hspace{0pt}%DIFAUXCMD
also studied the effect of upper cutoff voltage on lifetime and found that while the lifetime of cells with oxide-based positive electrode materials was at worst moderately sensitive to upper cutoff voltage over a range of 250 mV, LFP cells exhibited a sharp knee with a 50 mV increase in the upper cutoff voltage.
}\DIFaddend Other studies also observed accelerated degradation at extreme SOCs.\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{aiken_accelerated_2020,ma_novel_2019, zhu_investigation_2021}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ma_novel_2019, aiken_accelerated_2020, zhu_investigation_2021}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend 
...
Additionally, the nature of the three internal state trajectories that cause knees also pose major challenges (i.e., extrapolation of \DIFdelbegin \DIFdel{an exponential }\DIFdelend \DIFaddbegin \DIFadd{a superlinear }\DIFaddend function for snowball trajectories, simultaneously tracking two internal states for hidden trajectories, etc.).
...
A final complication is translating lab testing results to a variety of field usage conditions\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, which requires an accurate understanding of both real-world usage behavior and battery degradation as a function of infinite behavioral axes (e.g., distributions of temperature, state of charge, rest time, etc.); these challenges are only starting to be recognized by the academic community\mbox{%DIFAUXCMD
\cite{sulzer_challenge_2021}{}}\hspace{0pt}%DIFAUXCMD
, as access to real field data is an obvious challenge.
}\DIFaddend Given this complexity, modeling and predicting knees is undoubtedly a formidable task.

Despite these challenges, we remain optimistic about avenues for improving knee modeling and prediction. The primary goal of this work is to lay the foundation for a fundamental understanding of the physics of knees; this understanding enables an accurate assessment of the limits of today’s models and opportunities for future work.
Our findings suggest that \DIFdelbegin \DIFdel{many }\DIFdelend \DIFaddbegin \DIFadd{some }\DIFaddend knee pathways, such as electrode saturation or resistance growth, can be readily predicted from physics-driven or ``degradation mode'' (e.g., 'Alawa \cite{dubarry_synthesize_2012, dubarry_big_2020}) modeling \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{that uses cell-level electrochemical signals; for instance, Anse\'an et al.\mbox{%DIFAUXCMD
\cite{ansean_operando_2017}}\hspace{0pt}%DIFAUXCMD
, Sulzer et al.\mbox{%DIFAUXCMD
\cite{sulzer_accelerated_2021}}\hspace{0pt}%DIFAUXCMD
, and Mandli et al.\mbox{%DIFAUXCMD
\cite{mandli_analysis_2019} }\hspace{0pt}%DIFAUXCMD
demonstrated successful prediction of lithium plating, electrode saturation, and resistance growth knees, respectively, using bulk electrochemical state variables derived from differential capacity analysis and direct-current resistance. }\DIFaddend For other knee pathways, such as plating due to porosity decrease and additive depletion, the modeling is straightforward \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(e.g., Yang et al.\mbox{%DIFAUXCMD
\cite{yang_modeling_2017}}\hspace{0pt}%DIFAUXCMD
), }\DIFaddend but the methods to estimate \DIFdelbegin \DIFdel{internal state are not. }\DIFdelend \DIFaddbegin \DIFadd{the relevant internal state variables are not (e.g., they require laborious destructive measurements). }\DIFaddend We hope this work inspires future research into a comprehensive knee prediction toolkit that generalizes over a wide range of cell designs and use cases.

Lastly, we discuss the role of data-driven methods in this space.
Data-driven models may be well suited for knee pathways with bulk electrochemical signals; for instance, the high predictive performance of features sourced from the cell voltage responses during discharge in Severson et al.\cite{severson_data-driven_2019} was largely attributed to the detectability of the knee pathway via bulk electrochemical signals \DIFdelbegin \DIFdel{.
However, quantifying the values of bulk internal states from field data remains challenging\mbox{%DIFAUXCMD
\cite{aitio_predicting_2021, bian_state--health_2021, sulzer_challenge_2021}}\hspace{0pt}%DIFAUXCMD
.
Furthermore}\DIFdelend \DIFaddbegin \DIFadd{(i.e., measuring $\mathrm{LAM_{deNE}}$ via differential capacity analysis).
However}\DIFaddend , data-driven models trained on cycling data will naturally be poorly suited for knee pathways with signals that are challenging to measure via electrochemistry.
...
\DIFaddbegin \DIFadd{For instance, average charge and discharge voltage are readily accessible features that can indicate rapid changes in lithium inventory loss or internal resistance growth rates.\mbox{%DIFAUXCMD
\cite{harlow_use_2018}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend As a whole, datasets that span a variety of knee pathways for various cell designs and use cases are needed for training generalizable data-driven models.
Generation of synthetic data sets using physics-based models may be well suited for training generalizable models reliant on electrochemical signals to identify knee pathways or predict knees.\cite{dubarry_big_2020, kim_rapid_2021}
\DIFaddbegin \DIFadd{Furthermore, quantifying the values of bulk internal states from field data remains challenging\mbox{%DIFAUXCMD
\cite{aitio_predicting_2021, bian_state--health_2021, sulzer_challenge_2021}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
In particular, the ``covering layer'' phenomenon observed concurrently with knees in many studies deserves further \DIFaddbegin \DIFadd{experimental }\DIFaddend study.
...
Lastly, a variety of modeling and prediction approaches, spanning the physics-driven to data-driven continuum, may unlock accurate lifetime estimation to capture the most challenging lithium-ion battery degradation modes\DIFaddbegin \DIFadd{.
On the physics-driven end of the spectrum, multi-length-scale modeling will be required given that the length scales of knee-relevant degradation ranges from nanometers to centimeters; on the other end of the spectrum, large datasets spanning the vast cell design and usage space are imperative to advance data-driven modeling efforts}\DIFaddend .
