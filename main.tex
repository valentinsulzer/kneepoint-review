...
Furthermore, locating a knee by eye is seemingly straightforward for a single, smooth and ideal health profile (e.g.\, around 400 cycles in Figure \ref{fig:degradation_shapes}).
...
\caption{Definition of a knee point. (Sam to complete caption). Also please label the panels with a and b, and replace "cycles" with "Cycle number")}
...
\pbox{Propose moving this to later section}}
\label{fig:knees2EOL}
\end{figure}
...
Lithium plating occurs when lithium ions form metallic lithium on the surface of the electrode rather than intercalating into it. The plating reaction becomes favorable when the potential of the anode, given in equation \ref{plate}, drops below the Li/Li+ potential, generally defined to be zero. Many pathways for plating exist, and they can be broken into two categories. Thermodynamic plating can be defined as plating which is not heavily influenced by current. These pathways occur when the $V_{neg}$ term in equation \ref{plate} approaches zero against Li/Li+ as a result of thermal effects or as a result of the electrode approaching its lithium capacity. Kinetic plating occurs when current-induced conditions cause $\eta_{pl}$ to drop below zero. Typically, this means $\eta_{neg}$ is very low, however, large concentration gradients caused by high current in conditions which lower the diffusion coefficient (such as fast charging under cold ambient temperature) can cause $V_{neg}$ to drop locally. The remainder of this section enumerates the pathways by which plating can lead to a kneepoint. 

\begin{equation}
    \eta_{pl} = V_{neg}+\eta_{neg}-V_{pl}+R_{sei}I
    \label{plate}
\end{equation}
